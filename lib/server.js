...
process.on('unhandledRejection', (reason, p) => {
  logger.error(`encountered unhandled promise rejection at process level: ${p}, reason: ${reason.stack || reason}`);
});
...
async function initConfiguration(rawConfig) {
  try {
    ...
    if (!config.logger) {
      ...
      // TODO: global logger concept probably needs rewrite
      ({ logger } = config);
    }

    logger.debug(`configuration successfully read: ${config.configPath}`);
