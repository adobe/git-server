...
const {
  error, debug, info, rootLogger, FileLogger,
...
async function initConfiguration(rawConfig) {
  try {
    ...
    rootLogger.loggers.set( // Using a uuid in the name here makes collisions extremely unlikely
      ...
      new FileLogger(path.join(config.logs.logsDir, 'error.log'), { level: config.logs.level || 'info' }),
    );

    debug(`configuration successfully read: ${config.configPath}`);
  ...
  return new Promise((resolve, reject) => {
    const srv = http.createServer(app(config)).listen(port, host, (err) => {
      if (err) {
        ...
        info(`[${process.pid}] HTTP: listening on port ${srv.address().port}`);
        resolve(srv);
      }
    });
  });
    ...
    return new Promise((resolve, reject) => {
      const srv = https.createServer(options, app(config)).listen(port, host, (err) => {
        if (err) {
          ...
          info(`[${process.pid}] HTTPS: listening on port ${srv.address().port}`);
          resolve(srv);
        }
      });
    });
