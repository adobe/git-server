...
function applyDefaults(options) {
  options = options || {};
  options.repoRoot = options.repoRoot || DEFAULT_REPO_ROOT;

  options.listen = options.listen || {};
  options.listen.http = options.listen.http || {};
  options.listen.http.port = options.listen.http.port || DEFAULT_HTTP_PORT;
  options.listen.http.host = options.listen.http.host || DEFAULT_HOST;
  options.listen.https = options.listen.https || {};
  options.listen.https.port = options.listen.https.port || DEFAULT_HTTPS_PORT;
  options.listen.https.host = options.listen.https.host || DEFAULT_HOST;
  return options;
}
...
async function startHttpServer(config) {
  const host = config.listen.http.host;
  const port = config.listen.http.port;
...
async function startHttpsServer(config) {
  const host = config.listen.https.host;
  const port = config.listen.https.port;

  const createCertificate = util.promisify(pem.createCertificate);

  try {
    let options;
    if (config.listen.https.key && config.listen.https.cert) {
      options = {
        key: await fse.readFile(config.listen.https.key, 'utf8'),
        cert: await fse.readFile(config.listen.https.cert, 'utf8')
      };
    } else {
