    ...
    Git.Repository.open(repPath)
      .then(repo => {
        ...
        if (sha === 'master') {
          ...
          return _repo.getBranchCommit(sha)
            .catch(() => {
              // sha => commit id?
              // return _repo.getCommit(ref);
              // support shorthand commit id's
              return Git.AnnotatedCommit.fromRevspec(_repo, sha)
      ...
      .then(() => {
        ...
        commitLog.forEach(entry => {
          ...
          const url = `${req.protocol}://${host}/repos/${owner}/${repoName}/git/commits/${commit.sha()}`;
          result.push({
            sha: commit.sha(),
            node_id: NOT_IMPL,
            commit: {
