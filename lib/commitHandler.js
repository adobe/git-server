    ...
    Git.Repository.open(repPath)
      .then((repository) => {
        ...
        return repo.getBranchCommit(sha)
          .catch((err) => {
            ...
            options.logger.debug(`[commitHandler] errno: ${err.errno} errorFunction: ${err.errorFunction} message: ${err.message}`);
      ...
      .catch((err) => {
        if (err.errno === -3) {
          ...
          options.logger.debug(`[commitHandler] errno: ${err.errno} errorFunction: ${err.errorFunction} message: ${err.message} stack: ${err.stack}`);
          next(err);
        }
      })
      .finally(() => {
