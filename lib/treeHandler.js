...
function createMiddleware(options) {
  ...
  return (req, res, next) => {
    ...
    async function collectTreeEntries(tree, result, treePath, deep) {
      const entries = tree.entries.map(({
        oid, type, mode, path,
      }) => {
        return {
    ...
    resolveTree(repPath, refOrSha)
      ...
      .catch((err) => {
        ...
        if (err.code === 'ReadObjectFail' || err.code === 'ResolveRefError') {
          debug(`[treeHandler] resource not found: ${err.message}`);
          res.status(404).json({
            message: 'Not Found',
            documentation_url: 'https://developer.github.com/v3/git/trees/#get-a-tree',
          });
        } else {
          debug(`[treeHandler] code: ${err.code} message: ${err.message} stack: ${err.stack}`);
          next(err);
        }
      });
  };
