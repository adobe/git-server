    ...
    Git.Repository.open(repPath)
      .then((repository) => {
        ...
        return repo.head()
          ...
          .catch((err) => {
            ...
            options.logger.debug(`[rawHandler] errno: ${err.errno} errorFuntion: ${err.errorFunction} message: ${err.message}`);
      ...
      .catch((err) => {
        if (err.errno === -3) {
          ...
          options.logger.debug(`[rawHandler] errno: ${err.errno} errorFuntion: ${err.errorFunction} message: ${err.message} stack: ${err.stack}`);
          next(err);
        }
      })
      .finally(() => {
