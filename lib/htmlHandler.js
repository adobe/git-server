...
function createMiddleware(options, urlType) {
  ...
  return (req, res, next) => {
    ...
    let repPath = path.resolve(options.repoRoot, owner, repoName);

    // TODO: add global check for virtual repos
    if (options.virtualRepos[owner] && options.virtualRepos[owner][repoName]) {
      repPath = path.resolve(options.virtualRepos[owner][repoName].path);
    }
