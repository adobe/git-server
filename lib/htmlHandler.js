...
const { resolveCommit, determineRefPathName } = require('./git');
...
function createMiddleware(options, urlType) {
  ...
  return async (req, res, next) => {
    ...
    let refName = req.params.ref || 'master';
    ...
    resolveCommit(repPath, refName)
      ...
      .catch((err) => {
        debug(`[htmlHandler] code: ${err.code} message: ${err.message} stack: ${err.stack}`);
        next(err);
      });
  };
}
