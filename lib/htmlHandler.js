      ...
      .then((entry) => {
        if (!entry) {
          if (!fpath.length && urlType === 'root') {
            // 'root' view
            res.send(`<!DOCTYPE html><html><body>owner: ${owner}<br>repo: ${repoName}<br>ref: ${refName}<p>root view not implemented yet.</body></html>`);
          } else if (!fpath.length && urlType === 'tree') {
            // 'tree' view
            res.send(`<!DOCTYPE html><html><body>owner: ${owner}<br>repo: ${repoName}<br>ref: ${refName}<p>tree view not implemented yet.</body></html>`);
          } else {
            res.status(404).send(`not found: ${fpath}`);
          }
          return;
        }
        if (entry.isTree() && (urlType === 'tree')) {
          // directory view
          res.send(`<!DOCTYPE html><html><body>owner: ${owner}<br>repo: ${repoName}<br>ref: ${refName}<br>path: ${fpath}<p>directory view not implemented yet.</body></html>`);
        } else if (entry.isBlob() && urlType === 'blob') {
          // single file view
          res.send(`<!DOCTYPE html><html><body>owner: ${owner}<br>repo: ${repoName}<br>ref: ${refName}<br>path: ${fpath}<p>file view not implemented yet.</body></html>`);
        } else {
          res.status(404).send(`not found: ${fpath}`);
        }
      })
