    ...
    Git.Repository.open(repPath)
      .then((repository) => {
        ...
        return repo.getReference(refName)
          ...
          .catch((err) => {
            ...
            options.logger.debug(`[htmlHandler] errno: ${err.errno} errorFuntion: ${err.errorFunction} message: ${err.message}`);
      ...
      .catch((err) => {
        if (err.errno === -3) {
          ...
          options.logger.debug(`[htmlHandler] errno: ${err.errno} errorFuntion: ${err.errorFunction} message: ${err.message} stack: ${err.stack}`);
          next(err);
        }
      })
      .finally(() => {
