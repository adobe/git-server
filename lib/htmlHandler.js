...
function createMiddleware(options, urlType) {
  ...
  return (req, res, next) => {
    ...
    Git.Repository.open(repPath)
      .then((repository) => {
        ...
        return repo.getReference(refName)
          ...
          .catch((err) => {
            ...
            options.logger.debug(`[htmlHandler] errno: ${err.errno} errorFunction: ${err.errorFunction} message: ${err.message}`);
      ...
      .catch((err) => {
        if (err.errno === -3) {
          options.logger.debug(`[htmlHandler] resource not found: ${err.message}`);
          res.status(404).send('not found.');
        } else {
          options.logger.debug(`[htmlHandler] errno: ${err.errno} errorFunction: ${err.errorFunction} message: ${err.message} stack: ${err.stack}`);
          next(err);
        }
      })
      .finally(() => {
