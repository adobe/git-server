...
function createMiddleware(options, archiveFormat) {
  ...
  return (req, res, next) => {
    ...
    Git.Repository.open(repPath)
      .then((repository) => {
        ...
        return repo.head()
          ...
          .catch((err) => {
            ...
            options.logger.debug(`[archiveHandler] errno: ${err.errno} errorFunction: ${err.errorFunction} message: ${err.message}`);
      ...
      .catch((err) => {
        options.logger.debug(`[archiveHandler] errno: ${err.errno} errorFunction: ${err.errorFunction} message: ${err.message} stack: ${err.stack}`);
        next(err);
      })
      .finally(() => {
        // cleanup
