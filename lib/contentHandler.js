...
const { resolveCommit, getObject } = require('./git');
...
function createMiddleware(options) {
  ...
  return (req, res, next) => {
    ...
    resolveCommit(repPath, refName)
      .then((commitOid) => git.readObject({ dir: repPath, oid: commitOid, filepath: fpath }))
      .then(({ type, oid, object }) => {
        ...
        // dir
        return treeEntriesToJson(object.entries, fpath);
      })
      .then((json) => {
        res.json(json);
      })
      .catch((err) => {
        ...
        if (err.code === 'ResolveRefError') {
          debug(`[contentHandler] resource not found: ${err.message}`);
          res.status(404).json({
            message: `No commit found for the ref ${refName}`,
            documentation_url: 'https://developer.github.com/v3/repos/contents/',
          });
        } else if (err.code === 'TreeOrBlobNotFoundError') {
          debug(`[contentHandler] resource not found: ${err.message}`);
          res.status(404).json({
            message: 'Not Found',
          ...
          debug(`[contentHandler] code: ${err.code} message: ${err.message} stack: ${err.stack}`);
          next(err);
        }
      });
  };
