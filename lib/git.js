  ...
  return git.resolveRef({ fs, dir, ref })
    .catch(async (err) => {
      if (err instanceof git.Errors.NotFoundError) {
        ...
        const oid = await git.expandOid({ fs, dir, oid: ref, cache }).catch(() => { throw err; });
...
async function resolveBlob(dir, ref, pathName, includeUncommitted) {
  ...
  const status = await git.status({ fs, dir, filepath: pathName, cache });
...
async function createBlobReadStream(dir, oid) {
  const { object: content } = await git.readObject({ fs, dir, oid, cache });
...
async function getObject(dir, oid) {
  return git.readObject({ fs, dir, oid, cache });
}
...
async function resolveObject(dir, oid, pathName) {
  return git.readObject({ fs, dir, oid, filepath: pathName, cache });
}
...
async function resolveTree(dir, refOrSha) {
  ...
  if (isValidSha(refOrSha)) {
    ...
    try {
      ...
      if (err instanceof git.Errors.NotFoundError) {
        // fallback: is ref a shortened oid prefix?
        oid = await git.expandOid({ fs, dir, oid: refOrSha, cache }).catch(() => { throw err; });
      } else {
        // re-throw
        throw err;
      }
  ...
  return git.readObject({ fs, dir, oid, cache })
    .then((obj) => {
      ...
      if (obj.type === 'commit') {
        return git.readObject({ fs, dir, oid: obj.object.tree, cache });
      }
      if (obj.type === 'tag') {
        if (obj.object.type === 'commit') {
          return git.readObject({ fs, dir, oid: obj.object.object, cache })
            .then((commit) => git.readObject({ fs, dir, oid: commit.object.tree, cache }));
        }
